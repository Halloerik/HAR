...
def generate_data(dataset, target_filename, label):
    ...
    for filename in OPPORTUNITY_DATA_FILES:
        ...
        elif counter_files > 11 and  counter_files < 14:
            try:
                data = np.loadtxt(BytesIO(zf.read(filename)))
    ...
    # data_management is segmented into train and test
...
def get_args():
    ...
    # Add arguments
    parser.add_argument(
        '-i', '--input', type=str, help='OPPORTUNITY zip file', required=False)
    parser.add_argument(
        '-o', '--output', type=str, help='Processed data file', required=False)
    parser.add_argument(
        '-t', '--task', type=str.lower, help='Type of activities to be recognized', default="locomotion", choices = ["gestures", "locomotion"], required=False)
    # Array for all arguments passed to script
    args = parser.parse_args()
    # Assign args to variables
    
    dataset = "/home/fmoya/Documents/Doktorado/OrderPicking/DeepConvLSTM/OpportunityUCIDataset.zip"
    #dataset = args.input
    #target_filename = args.output
    target_filename = "train_val_test_dataset_locomotion.data"
    label = args.task
